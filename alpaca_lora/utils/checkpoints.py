...
class ExportCheckpointConfig(Config):
    base_model: Union[str, Path]
    lora_weights: Union[str, Path]
    output_dir: Union[str, Path]

def export_alpaca_lora_checkpoint(config: ExportCheckpointConfig):
    # Lazily import to speed up import time
    import torch
    import transformers
    ...
    torch.save(
        new_state_dict,
        os.path.join(config.output_dir, "consolidated.00.pth")
    )
